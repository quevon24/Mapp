...
class Usuario_audios(forms.ModelForm):
	...
	class Meta:
		model = Perfil_audio
		fields = ('descripcion', 'file_ids', 'contacto')
		widgets = {
		'user': forms.HiddenInput(),
		}

	def __init__(self, *args, **kwargs):
		super(Usuario_audios, self).__init__(*args, **kwargs)
		self.fields['contacto'].queryset = Contactos.objects.filter(
										user_id=self.instance.user)

	def save(self, commit=True):
		media = super(Usuario_audios, self).save(commit=False)
		if commit:
			media.save()
			print self.instance.pk
			inst = self.instance
			...
			for file_id in file_ids:
				print file_id  
				f = Perfil_audio_archivo.objects.get(id=file_id)
				f.update(carta=inst)
				#a = FilesUpload(section=inst,attachment=f)
				#a.save()
		return media
...
class Usuario_videos(forms.ModelForm):
	...
	class Meta:
		model = Perfil_video
		fields = ('descripcion', 'file_ids', 'contacto', 'formato')
		widgets = {
		'user': forms.HiddenInput(),
		}

	def __init__(self, *args, **kwargs):
		super(Usuario_videos, self).__init__(*args, **kwargs)
		self.fields['contacto'].queryset = Contactos.objects.filter(
										user_id=self.instance.user)

	def save(self, commit=True):
		media = super(Usuario_videos, self).save(commit=False)
		if commit:
			media.save()
			print self.instance.pk
			inst = self.instance
			...
			for file_id in file_ids:
				print file_id  
				f = Perfil_video_archivo.objects.get(id=file_id)
				f.update(carta=inst)
				#a = FilesUpload(section=inst,attachment=f)
				#a.save()
		return media
